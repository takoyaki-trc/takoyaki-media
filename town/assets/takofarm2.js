/* takofarm2.js （テスト用：開設記念シリアル “anniv” 対応版）
   ✅ 変更点（重要）
   - seed id を「seed_anniversary」に統一（※seed_anniv という表記は使わない）
   - callSerial に apiKey を必ず付与（GAS側が必須）
   - anniv植えた瞬間に GASへ plant（PLANTED記録）
   - anniv収穫は GASの戻り { ok:true, result:{card_id:"..."} } を読む
   - 開設記念カード画像は「絶対URL」で固定（GitHub Pagesの相対事故回避）
*/
...
(() => {
  ...
  // =========================
  const SERIAL_API_URL =
    "https://script.google.com/macros/s/AKfycbwXJXFLCgL7ZMgb7M1hHwfKI6vBPicWgf0yutF5qyo9fkLrGH393zSoA20sRqk7PO71/exec";

  // ✅ GAS側CONFIG.API_KEYと一致させる
  const API_KEY = "takopi-gratan-2026";

  // ✅ 絶対URLの基準（開設記念画像用）
  const TOWN_BASE =
    "https://takoyaki-trc.github.io/takoyaki-media/town/";
  ...
  const SEEDS = [
    ...
    // ✅ 開設記念（シリアル専用）
    {
      ...
      factor: 1.0,
      img: TOWN_BASE + "assets/images/anniversary/anv1.png",
      fx: "記念イベント専用",
    },
  ];
  ...
  // =========================
  // ✅ 開設記念：card_id → 画像/レア対応（絶対URL）
  // =========================
  const ANNIV_CARD_MAP = {
    "ANN-N-001": { name: "開設記念 N", rarity: "N", img: TOWN_BASE + "assets/images/anniversary/1.png" },
    "ANN-R-001": { name: "開設記念 R", rarity: "R", img: TOWN_BASE + "assets/images/anniversary/2.png" },
    "ANN-SR-001": { name: "開設記念 SR", rarity: "SR", img: TOWN_BASE + "assets/images/anniversary/3.png" },
    "ANN-UR-001": { name: "開設記念 UR", rarity: "UR", img: TOWN_BASE + "assets/images/anniversary/4a.jpg" },
    "ANN-LR-001": { name: "開設記念 LR", rarity: "LR", img: TOWN_BASE + "assets/images/anniversary/4b.jpg" },
  };
  ...
  function cardFromCardId(cardId) {
    ...
    return { id: cardId, name: String(cardId || "UNKNOWN"), img: PLOT_IMG.EMPTY, rarity: "" };
  }
  ...
  function saveSeedTokens(x) {
    localStorage.setItem(LS_SEEDTOKENS, JSON.stringify(x));
  }
  ...
  function itemRewardForLevel(level) {
    ...
    // ✅ シリアル種はレベル報酬では増やさない
    const seedChoices = SEEDS.filter((x) => x.id !== "seed_colabo" && x.id !== "seed_anniversary");
  ...
  function drawRewardForPlot(p) {
    ...
    // ✅ seed_anniversary はここで抽選しない（GAS確定）
    if (p && p.seedId === "seed_anniversary") {
      return null;
    }
  ...
  // =========================================================
  // ✅ 装備表示更新（seed_anniversary は token 数を表示）
  // =========================================================
  function renderLoadout() {
    ...
    if (seed) {
      ...
      const cnt = seed.id === "seed_anniversary" ? tokenCount("anniv") : invGet(inv, "seed", seed.id);
  ...
  // =========================================================
  // ✅ グリッド選択UI（seed_anniversary は token 数で可否判定）
  // =========================================================
  function openPickGrid(kind) {
    ...
    const cells = items
      .map((x) => {
        ...
        // ✅ シリアル種だけ tokenCount で表示
        if (isSeed && x.id === "seed_anniversary") {
          cnt = tokenCount("anniv");
        }
    ...
    openModal(
      title,
      `
      <div class="step">※すべて在庫制。露店で買って増やす。<br>装備は消費しない（植えた時に消費）。<br>
      ※<b>開設記念のタネ</b>はシリアルの <b>seed_token</b> 回数です。</div>
  ...
  async function plantAt(index) {
    ...
    // ✅ anniv は inv じゃなく token
    let seedToken = null;
    let serialReward = null;

    if (seedId === "seed_anniversary") {
      serialReward = "anniv";
      ...
      if (!seedToken) {
        openModal(
          "在庫が足りない",
          `
          <div class="step"><b>開設記念のタネ</b>の seed_token がありません。<br>先にシリアルredeemしてください。</div>
    ...
    const okSeed = seedId === "seed_anniversary" ? true : invGet(inv, "seed", seedId) > 0;
    ...
    const isFixedSeed =
      ...
      seedId === "seed_anniversary";

    // annivはサーバー確定なので固定レアは持たない
    ...
    // ✅ 先に保存してUI更新（失敗時も復帰しやすい）
    state.plots[index] = plot;
    saveState(state);
    render();

    // ✅ annivは植えた瞬間にGASへ plant（B-1方式の整合性）
    if (seedId === "seed_anniversary") {
      try {
        const r = await callSerial("plant", { seed_token: seedToken });
        if (!r || !r.ok) {
          // ここで戻す（トークンは消費済みなので、戻したいならtokenを戻す必要がある）
          openModal(
            "植え付けエラー",
            `
            <div class="step">GASのplantに失敗しました。<br>${(r && r.error) ? r.error : ""}</div>
            <div class="row"><button type="button" id="btnOk" class="primary">OK</button></div>
          `
          );
          clearHarvestCommit();
        ...
        openModal(
          "植え付けエラー",
          `
          <div class="step">通信に失敗しました。GAS URL / 公開設定を確認してください。</div>
          <div class="row"><button type="button" id="btnOk" class="primary">OK</button></div>
        `
        );
        clearHarvestCommit();
  ...
  // =========================================================
  // ✅ マス操作（READYでGAS問い合わせがあるので async）
  // =========================================================
  async function onPlotTap(i) {
    ...
    // =========================================================
    // ✅ READY：anniv は収穫時にGASへ問い合わせ
    // =========================================================
    if (p.state === "READY") {
      if (p.seedId === "seed_anniversary") {
        // まだ確定結果が無いなら取りに行く
        if (!p.reward || !p.reward.id || !String(p.reward.id).startsWith("ANN-")) {
          try {
            const r = await callSerial("harvest", { seed_token: p.seedToken });

            // ✅ GASは { ok:true, result:{card_id:"..."} } の形
