/* takofarm2.js （テスト用：開設記念シリアル “anniv” 対応版）
   ✅ 変更点
   - シリアル種 seed_anniv を追加（invではなく seed_token で回数管理）
   - seed_tokens を localStorage(tf_v1_seedtokens) に保持
   - 植える時に seed_token を1本消費して plot.seedToken に保存
   - 収穫時に GAS へ seed_token を渡して card_id を取得→画像/レアへ変換
   - 既存の畑抽選（WATER等）は通常種のまま。annivだけGAS確定
*/
...
(() => {
  ...
  // （あなたの指定：1=N,2=R,3=SR,4a=UR,4b=LR）
  // =========================
  ...
  // =========================================================
  // ✅ 装備表示更新（seed_anniv は token 数を表示）
  // =========================================================
  ...
  // =========================================================
  // ✅ グリッド選択UI（seed_anniv は token 数で可否判定）
  // =========================================================
  ...
  function plantAt(index){
    ...
    const fixedRarity = (isFixedSeed && seedId !== "seed_anniv") ? null : (isFixedSeed ? null : pickRarityWithWater(waterId));
    // anniv は GAS確定なので hintは出さない
    const srHint =
      (seedId === "seed_anniv") ? "NONE" :
      (isFixedSeed) ? "NONE" :
      ...
      if(p.seedId === "seed_anniv"){
        ...
        if(!p.reward || !p.reward.id || !String(p.reward.id).startsWith("ANN-")){
          try{
            ...
            p.reward = cardFromCardId(r.card_id);
            saveState(state);
          }catch(e){
            openModal("収穫エラー", `
