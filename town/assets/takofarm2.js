...
(() => {
  ...
  const GAS_URL = "https://script.google.com/macros/s/AKfycbw3FShwFkw5mdlUUBcnZXjcqIBmB-OoNEAf-XoYdDqhJjMS0CtcxVa57FURofzdeoZ1/exec";
  ...
  const COLLAB_BY_SEEDID = Object.fromEntries(
    COLLAB_SEEDS.map(x => [String(x.seedId), String(x.collabId)])
  );
  ...
  const FERTS = [
    ...
    { id:"fert_timeno", name:"時間を信じない肥料", desc:"最終兵器・禁忌。\n（今は時短だけ）", factor:0.10, fx:"時短 70%", img:"https://ul.h3z.jp/l2njWY57.png", burnCardUp:0.00, rawCardChance:0.03, mantra:false, skipGrowAnim:true },
  ];
  ...
  function savePlayer(p){
    localStorage.setItem(LS.player, JSON.stringify(p));
  }
  ...
  function saveLoadout(l){
    localStorage.setItem(LS.loadout, JSON.stringify(l));
  }
