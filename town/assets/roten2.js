...
(() => {
  ...
  function toastHype(text, opt = {}) {
    ...
    if (kind === "good") {
      ...
      el.style.setProperty(
        "box-shadow",
        "0 18px 44px rgba(0,0,0,.55), 0 0 22px rgba(159,255,168,.18)",
        "important"
      );
    } else if (kind === "bad") {
      ...
      el.style.setProperty(
        "box-shadow",
        "0 18px 44px rgba(0,0,0,.55), 0 0 22px rgba(255,154,165,.16)",
        "important"
      );
    } else {
      el.style.setProperty("border", "1px solid rgba(255,255,255,.16)", "important");
  ...
  function openBreakdownModal(kindKey) {
    ...
    if (kindKey === "seed") {
      ...
      const detail = keys.length
        ? `<div class="note" style="margin-top:10px; opacity:.82;">未使用token内訳：${keys
            .map((k) => `${escapeHTML(k)}×${by[k]}`)
            .join(" / ")}</div>`
  ...
  function wireChipBreakdowns() {
    $("#chipSeed")?.addEventListener("click", () => {
      openBreakdownModal("seed");
      setTakopiSayRandom();
    });
    $("#chipWater")?.addEventListener("click", () => {
      openBreakdownModal("water");
      setTakopiSayRandom();
    });
    $("#chipFert")?.addEventListener("click", () => {
      openBreakdownModal("fert");
      setTakopiSayRandom();
    });
  }
  ...
  async function gasPost(payload) {
    ...
    try {
      ...
      throw new Error("JSON parse failed: " + txt);
    }
    return json;
  }
  ...
  function openMikuji() {
    ...
    if (done) {
      openModal(
        "🎲 たこ焼きみくじ",
        `<div class="mikuji-wrap"><div class="note">今日はもう引いた…たこ。明日またおいで…たこ。</div></div>`
      );
      return;
    }
  ...
  function openLaunchPresent() {
    ...
    if (claimed) {
      openModal(
        "🎁 公開記念プレゼント",
        `<div class="mikuji-wrap"><div class="note">もう受け取った…たこ。大事に使って…たこ。</div></div>`
      );
      return;
    }
